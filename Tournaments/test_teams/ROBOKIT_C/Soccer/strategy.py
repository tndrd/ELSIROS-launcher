...
class Forward:
    ...
    def __init__(self, logger, motion, local, glob):
        ...
        self.glob = glob
        self.direction_To_Guest = 0
        self.first_doing = True

    def go_back_10_steps(self):
        if self.first_doing:
            self.motion.near_distance_omni_motion(self, -1000, 0)
        self.first_doing = False
...
class Player():
    ...
    def forward_main_cycle(self, pressed_button):
        ...
        # self.motion.near_distance_omni_motion(-100,0)
        # self.motion.near_distance_omni_motion(-100, 0)
        success_Code, napravl, dist, speed = self.motion.seek_Ball_In_Pose(fast_Reaction_On=True)
        small_kick = True
        success_Code = self.motion.near_distance_ball_approach_and_kick(self.f.direction_To_Guest+math.pi/4, strong_kick=False,
                                                                        small_kick=small_kick)
        self.f.dir_To_Guest()
        self.logger.info("---------------------------------------------------------------")
        ...
        if pressed_button == 1:
            self.first_kick = 1
            self.motion.near_distance_omni_motion(150, 0.7)

        first_shoot_angle = math.pi / 3 * (-1) ** random.randint(0, 1)
        self.first_kick = 1
        ...
        while (True):
            ...
            player_from_ball_yaw = self.motion.p.coord2yaw(self.glob.pf_coord[0] - self.glob.ball_coord[0],
                                                          self.glob.pf_coord[1] - self.glob.ball_coord[1]) - (self.f.direction_To_Guest +  first_shoot_angle * self.first_kick)
            ...
            if (dist > 0.35  or player_in_front_of_ball) and not player_in_fast_kick_position:
                ...
                self.motion.far_distance_plan_approach(self.glob.ball_coord, self.f.direction_To_Guest + first_shoot_angle * self.first_kick, stop_Over = stop_Over)
            ...
            success_Code = self.motion.near_distance_ball_approach_and_kick(self.f.direction_To_Guest + first_shoot_angle * self.first_kick, strong_kick = False, small_kick = small_kick)
    ...
    def forward_old_style_main_cycle(self, pressed_button):
        ...
        while (True):
            ...
            if self.first_kick == True:
                self.f.direction_To_Guest += math.pi/4
                self.first_kick = False
