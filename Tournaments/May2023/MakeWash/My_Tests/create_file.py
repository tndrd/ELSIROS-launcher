...
def create_json(dir_path='Gk_sync',ball_coord=[],robot_coord=[],robot_current_state=''):
    filename = os.path.join(dir_path,f'params_{datetime.now().microsecond}.json')

    json_str  = f'{{"ball_coord": {{"X": {ball_coord[0]}, "Y": {ball_coord[1]}}}, "robot_coord": {{"X": {robot_coord[0]}, "Y": {robot_coord[1]}}}, "robot_current_state": "{robot_current_state}"}}'
