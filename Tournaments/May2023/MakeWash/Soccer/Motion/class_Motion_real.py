...
#доп функционал по выбору начального сектора при поиске мяча
from .segment_juggle import sort_list
...
#--------------------------------------------------------------------------------------------!!!!!
#--------------------------------------------------------------------------------------------!!!!!
    def seek_Ball_In_Pose(self, fast_Reaction_On, penalty_Goalkeeper = False, with_Localization = True,start_segment_Viev=2):
        ...
        #Сортируем список с учетом установки в его начала значения сегмента в котором мяч был обнаружен в прошлый раз
        #head_pose_seq = sort_list(head_pose_seq,start_segment_Viev)
        head_pose_seq.insert(0, start_segment_Viev)
        ...
        for i in range(len(head_pose_seq)):
            ...
            if a == True: 
                ...
                #Присваиваем start_segment_Viev номер сегмента в котором был найден мяч!
                start_segment_Viev = head_pose_seq[i]
                print(F"Найден мяч в сегменте {start_segment_Viev}")
        ...
        if len(variants)>0:
            ...
            if a == True:
                ...
                #возвращаем помимо остального, номер сегмента в котором нашли мяч!
                return(a, course, dist, speed,start_segment_Viev)
            else:
                if distance1 !=0:
                    if with_Localization: self.local.localisation_Complete()
                    dist = distance1 / 1000
    ...
    def near_distance_ball_approach_and_kick(self, kick_direction, strong_kick = False, small_kick = False,start_segment_Viev =2 ):
        offset_of_ball = self.params['KICK_OFFSET_OF_BALL']  # self.d10 # module of local robot Y coordinate of ball im mm before kick 
        a, napravl, dist, speed,start_segment_Viev = self.seek_Ball_In_Pose(fast_Reaction_On = True,start_segment_Viev =start_segment_Viev)
        dist_mm = dist *1000
