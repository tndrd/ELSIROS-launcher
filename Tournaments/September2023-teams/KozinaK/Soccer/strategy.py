...
class Player():
    ...
    def forward_main_cycle(self, pressed_button):
        ...
        usage:
            self.forward_main_cycle(int: pressed_button)
                                                                """
        self.logger.info("\n\nKozinaK team\n\n")
        second_player_timer = self.motion.game_time()                       # игнор в начале игры
        self.f = Forward_Vector_Matrix(self.logger, self.motion, self.local, self.glob) # загрузка матрицы какой-то, наверное та которая в strategy designer
        while True:
            if self.motion.falling_Flag != 0:                               # falling_Flag variable is used to with purpose to deliver
                                                                            ...
                                                                            # Да, короче обработка падений
                if self.motion.falling_Flag == 3: # Если попросили остановаиться
                    self.motion.play_Soft_Motion_Slot(name = 'Initial_Pose') # Занять стартовое
                    break # И выйти из цикла
                self.motion.falling_Flag = 0 
                self.local.coordinate_fall_reset()                          # поднимает робота, где-то там
            success_Code, napravl, dist, speed = self.motion.seek_Ball_In_Pose(fast_Reaction_On = True) # Ищет мяч
            ...
            self.f.dir_To_Guest()                                           # Тут точно уже загрузка из strategy designer
            ...
            if dist == 0 and success_Code == False:                         # не нашел мяч
                self.motion.turn_To_Course(self.glob.pf_coord[2]+ 2 * math.pi / 4)  # робот поварачивается на 1/3 ориентации
                continue
            ...
            small_kick = False
            if self.f.kick_Power == 3: small_kick = True
            strong_kick = False
    ...
    def goalkeeper_old_style_main_cycle(self):
        ...
        self.logger.debug("KozinaK Team")
        while (True):
            dist = -1.0
            if self.motion.falling_Flag != 0:                               # falling_Flag variable is used to with purpose to deliver
                                                                            # to strategy falling event or request to stop cycle. 
