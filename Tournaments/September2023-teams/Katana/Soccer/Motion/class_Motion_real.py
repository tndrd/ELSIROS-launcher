...
#--------------------------------------------------------------------------------------------!!!!!
    def seek_Ball_In_Pose(self, fast_Reaction_On, penalty_Goalkeeper = False, with_Localization = True,start_segment_Viev=2):
        ...
        #head_pose_seq = [0,1,2,3,4,9,8,7,6,5,10,11,12,13,14]
        head_pose_seq.insert(0, start_segment_Viev)
        self.logger.info(f"Sectors {head_pose_seq}")
        #print(f"Sectors {head_pose_seq}")
        ...
        for i in range(len(head_pose_seq)):
            ...
            if a == True: 
                ...
                self.logger.info(f"Find sector {start_segment_Viev}")
                #print(f"Find sector {start_segment_Viev}")
        ...
        if len(variants)>0:
            ...
            if a == True:
                ...
                if distance1 !=0:
                    ...
                    self.glob.ball_coord = [dist * math.cos(course_global_rad) + self.glob.pf_coord[0],
                                            dist * math.sin(course_global_rad) + self.glob.pf_coord[1]]
                    return(a, course1, dist, [0, 0],start_segment_Viev)
        if with_Localization: self.local.localisation_Complete()
        return False, 0, 0, [0, 0],2
    ...
    def near_distance_ball_approach_and_kick(self, kick_direction, strong_kick = False, small_kick = False,start_segment_Viev =2,dist_correct =0 ):
        offset_of_ball = self.params['KICK_OFFSET_OF_BALL']  # self.d10 # module of local robot Y coordinate of ball im mm before kick 
        a, napravl, dist, speed,start_segment_View = self.seek_Ball_In_Pose(fast_Reaction_On = True,start_segment_Viev =start_segment_Viev)
        dist_mm = dist *1000-dist_correct
