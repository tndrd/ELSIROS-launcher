...
goalPostRadius = 0.15      # Радиус обхода стойки ворот (in m).
ballRadius = 0.1           # Радиус обхода мяча (in m).
#-------------------------------------------------------------------------------
uprightRobotRadius = 0.5  # Радиус обхода вертикального робота (in m). было 0.2
#-------------------------------------------------------------------------------
...
class PathPlan:
    """
    Планирует оптимизированный путь робота-гуманоида от начальной координаты до целевой координаты.
    Координаты берутся вместе с ориентацией.
    Путь состоит из начальной дуги, конечной дуги и соединительной линии.
    Соединительная линия должна быть касательной к дугам.
    В случае наличия препятствий на линии пути добавляется дополнительная дуга для обхода препятствия.
    Только одно препятствие можно надежно избежать. Обход второго препятствия не гарантируется.
    Поэтому используются оценки цен вариантов пути. Путь с более дешевым
    цена возвращается. Столкновение с препятствием на дальнем расстоянии дешевле, чем столкновение с
    препятствие на близком расстоянии.
    Во время эвристики пути учитываются различные радиусы дуг. Дуга с нулевым радиусом означает
    поворот без изменения координат.
    """
